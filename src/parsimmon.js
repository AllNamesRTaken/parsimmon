...
class Parsimmon {
  ...
  assert(condition, errorMessage) {
    return this.chain((value) =>
      condition(value) ? Parsimmon.succeed(value) : Parsimmon.fail(errorMessage)
    )
  }
  ...
  times(min, max) {
    ...
    return new Parsimmon((input, i) => {
      ...
      return Parsimmon.mergeReplies(Parsimmon.makeSuccess(i, accum), prevResult)
    })
  }
  ...
  static seqObj() {
    ...
    for (let j = 0; j < numParsers; j += 1) {
      ...
      if (Parsimmon.isArray(p)) {
        const isWellFormed =
          p.length === 2 && typeof p[0] === 'string' && Parsimmon.isParser(p[1])
        if (isWellFormed) {
          const key = p[0]
...
// Replace CommonJS export with ES5 export
